...
class UserForm(forms.ModelForm):
    date_of_birth = forms.DateField(error_messages = {'invalid': 'Date should be in format (yyyy-mm-dd) eg: 2019-05-05'})
    class Meta:
        model = Customer
        fields = '__all__'
        exclude = ['id_front', 'id_back']
        widgets = {'user': forms.HiddenInput(), 'referrer': forms.HiddenInput(), 'id_front': forms.HiddenInput(), 'id_back': forms.HiddenInput(), 'gender': forms.HiddenInput()}
    ...
    def clean(self):
        ...
        try:
            ...
            if password1 != act.passworld:
                self.add_error('password1', "Password is not correct")
