...
class UserForm(forms.ModelForm):
    phone_number = forms.CharField(min_length=9, error_messages={
                                   'invalid': 'Enter Phone number in international format'})

    class Meta:
        model = Customer
        fields = '__all__'
        widgets = {'user': forms.HiddenInput(
        ), 'referrer': forms.HiddenInput(), 'gender': forms.HiddenInput()}

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['country'].widget.attrs.update(
            {'class': 'form-control', 'placeholder': 'Enter Country/Region'})
        self.fields['city'].widget.attrs.update(
            {'class': 'form-control', 'placeholder': 'Enter City/Town'})
        self.fields['address'].widget.attrs.update(
            {'class': 'form-control', 'rows': 1, 'placeholder': 'Address line'})
        self.fields['phone_number'].widget.attrs.update(
            {'class': 'form-control', 'placeholder': 'Phone in International Format'})
        self.fields['date_of_birth'].widget.attrs.update(
            {'class': 'form-control', 'placeholder': '(yyyy-mm-dd) eg: 2019-05-05'})
...
class PasswordChangeForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['password3'].widget = forms.PasswordInput(
            attrs={'class': 'form-control', 'placeholder': 'Confirm New Passwor'})
