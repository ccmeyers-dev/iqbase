...
from broker.models import Trade, Deposit, Customer, Wallet

class WalletForm(forms.ModelForm):
    class Meta:
        model = Wallet
        fields = '__all__'
        widgets = {'coin': forms.HiddenInput(), 'code': forms.HiddenInput(), 'hue': forms.HiddenInput()}
...
class coinform(forms.ModelForm):
    class Meta:
        model = Trade
        fields = '__all__'
        widgets = {'customer': forms.HiddenInput(), 'profit': forms.HiddenInput()}

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['wallet'].widget.attrs.update({'class': 'form-control', 'hidden':'hidden'})
        self.fields['amount'].widget.attrs.update({'class': 'form-control'})
        self.fields['duration'].widget.attrs.update({'class': 'form-control'})
